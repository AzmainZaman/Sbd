...
export default function ProductPage({ params }: PageProps) {
  ...
  const shipment = product.shipmentId
    ? shipments.find((s) => s.id === product.shipmentId) ?? null
    : null;
  ...
  return (
    <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav className="hidden lg:flex items-center gap-2 text-[13px] text-muted mb-8">
        ...
        <span>/</span>
        <Link href={`/categories/${product.category}`} className="hover:text-ink transition-colors capitalize">
          {product.category}
        </Link>
        <span>/</span>
      ...
      <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className="flex flex-col gap-5">
          ...
          {isPreOrder && shipment && (
            <ETABlock shipment={shipment} eta={product.eta!} />
          )}
